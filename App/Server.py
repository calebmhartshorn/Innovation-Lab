...
def add_quantity(barcode):
   ...
   if barcode in inventory:
       ...
       #Prints a message if the barcode is found
       ...
       print(f"Added {inventory_entry['name']}.")
   else:
       #Adds a new barcode entry to the inventory with unknown details
       inventory[barcode] = {
           "name": "Unknown",
...
def decode_barcode_from_webcam():
   ...
   while True:
       ...
       for obj in decoded_objects:
           ...
           #Keeps track of the number of times a barcode has been scanned
           if barcode_int in scanned_barcodes:
               scanned_barcodes[barcode_int] += 1
           else:
               scanned_barcodes[barcode_int] = 1

           #Adds the barcode to the inventory after it has been scanned 15 times
           ...
           #Draws a rectangle and text on the frame for the detected barcode
